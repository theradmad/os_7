#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
#include <string.h>
#include "list.h"
#include "util.h"
void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy);
list_t *coalese_memory(list_t *list);  // Corrected function name
void print_list(list_t *list, char *message);
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy);
void node_free(node_t *node);

void TOUPPER(char * arr){
  
    for(int i=0;i<strlen(arr);i++){
        arr[i] = toupper(arr[i]);
    }
}

void get_input(char *args[], int input[][2], int *n, int *size, int *policy) 
{
  	FILE *input_file = fopen(args[1], "r");
	  if (!input_file) {
		    fprintf(stderr, "Error: Invalid filepath\n");
		    fflush(stdout);
		    exit(0);
	  }

    parse_file(input_file, input, n, size);
  
    fclose(input_file);
  
    TOUPPER(args[2]);
  
    if((strcmp(args[2],"-F") == 0) || (strcmp(args[2],"-FIFO") == 0))
        *policy = 1;
    else if((strcmp(args[2],"-B") == 0) || (strcmp(args[2],"-BESTFIT") == 0))
        *policy = 2;
    else if((strcmp(args[2],"-W") == 0) || (strcmp(args[2],"-WORSTFIT") == 0))
        *policy = 3;
    else {
       printf("usage: ./mmu <input file> -{F | B | W }  \n(F=FIFO | B=BESTFIT | W-WORSTFIT)\n");
       exit(1);
    }
        
}

void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy) {
    block_t *allocated_block = NULL;

    // Find a suitable free block based on the policy
    switch (policy) {
        case 1: // First Fit
            allocated_block = list_find_first_fit(freelist, blocksize);
            break;
        case 2: // Best Fit
            allocated_block = list_find_best_fit(freelist, blocksize);
            break;
        case 3: // Worst Fit
            allocated_block = list_find_worst_fit(freelist, blocksize);
            break;
        default:
            fprintf(stderr, "Error: Invalid Memory Allocation Policy\n");
            exit(1);
    }

    if (allocated_block == NULL) {
        fprintf(stderr, "Error: Memory Allocation %d blocks\n", blocksize);
        exit(1);
    }

    // Remove the allocated block from the free list
    list_remove_node(freelist, allocated_block);

    // Set the block details for allocation
    allocated_block->pid = pid;
    allocated_block->end = allocated_block->start + blocksize - 1;

    // Add the allocated block to the allocated list in ascending order by address
    list_add_ascending_by_address(alloclist, allocated_block);

    // Deal with remaining memory (fragment)
    block_t *fragment = malloc(sizeof(block_t));
    fragment->pid = 0;
    fragment->start = allocated_block->end + 1;
    fragment->end = allocated_block->end;

    // Add the fragment back to the free list based on policy
    list_add_by_policy(freelist, fragment, policy);
}

list_t *coalese_memory(list_t *list) {
    list_t *temp_list = list_alloc();
    block_t *blk;

    // Sort the list in ascending order by address
    while ((blk = list_remove_from_front(list)) != NULL) {
        list_add_ascending_by_address(temp_list, blk);
    }

    // Try to combine physically adjacent blocks
    list_coalese_nodes(temp_list);

    return temp_list;
}


void print_list(list_t * list, char * message){
    node_t *current = list->head;
    block_t *blk;
    int i = 0;
  
    printf("%s:\n", message);
  
    while(current != NULL){
        blk = current->blk;
        printf("Block %d:\t START: %d\t END: %d", i, blk->start, blk->end);
      
        if(blk->pid != 0)
            printf("\t PID: %d\n", blk->pid);
        else  
            printf("\n");
      
        current = current->next;
        i += 1;
    }
}

void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy) {
    // Find the allocated block with the specified PID
    block_t *allocated_block = list_find_allocated_block(alloclist, pid);

    if (allocated_block == NULL) {
        fprintf(stderr, "Error: Can't locate Memory Used by PID: %d\n", pid);
        exit(1);
    }

    // Remove the allocated block from the allocated list
    list_remove_node(alloclist, allocated_block);

    // Set the block's PID back to 0
    allocated_block->pid = 0;

    // Add the block back to the free list based on policy
    list_add_by_policy(freelist, allocated_block, policy);
}

void list_coalese_nodes(list_t *l) {
    node_t *current = l->head;

    while (current != NULL && current->next != NULL) {
        if (current->blk->end + 1 == current->next->blk->start) {
            // Combine physically adjacent blocks
            current->blk->end = current->next->blk->end;
            node_t *temp = current->next;
            current->next = temp->next;
            node_free(temp);
        } else {
            current = current->next;
        }
    }
}

void list_add_by_policy(list_t *l, block_t *blk, int policy) {
    // This is a placeholder implementation, you need to adapt it based on your policies
    if (policy == 1) {
        list_add_ascending_by_blocksize(l, blk);
    } else if (policy == 2) {
        list_add_descending_by_blocksize(l, blk);
    } else {
    }
}

block_t *list_find_allocated_block(list_t *l, int pid) {
    if (l == NULL || l->head == NULL) {
        return NULL; // Empty list or invalid list
    }

    node_t *current = l->head;

    while (current != NULL) {
        if (current->blk->pid == pid) {
            // Found a block with the specified PID
            return current->blk;
        }
        current = current->next;
    }

    // No block with the specified PID found
    return NULL;
}


/* DO NOT MODIFY */
int main(int argc, char *argv[]) 
{
   int PARTITION_SIZE, inputdata[200][2], N = 0, Memory_Mgt_Policy;
  
   list_t *FREE_LIST = list_alloc();   // list that holds all free blocks (PID is always zero)
   list_t *ALLOC_LIST = list_alloc();  // list that holds all allocated blocks
   int i;
  
   if(argc != 3) {
       printf("usage: ./mmu <input file> -{F | B | W }  \n(F=FIFO | B=BESTFIT | W-WORSTFIT)\n");
       exit(1);
   }
  
   get_input(argv, inputdata, &N, &PARTITION_SIZE, &Memory_Mgt_Policy);
  
   // Allocated the initial partition of size PARTITION_SIZE
   
   block_t * partition = malloc(sizeof(block_t));   // create the partition meta data
   partition->start = 0;
   partition->end = PARTITION_SIZE + partition->start - 1;
                                   
   list_add_to_front(FREE_LIST, partition);          // add partition to free list
                                   
   for(i = 0; i < N; i++) // loop through all the input data and simulate a memory management policy
   {
       printf("************************\n");
       if(inputdata[i][0] != -99999 && inputdata[i][0] > 0) {
             printf("ALLOCATE: %d FROM PID: %d\n", inputdata[i][1], inputdata[i][0]);
             allocate_memory(FREE_LIST, ALLOC_LIST, inputdata[i][0], inputdata[i][1], Memory_Mgt_Policy);
       }
       else if (inputdata[i][0] != -99999 && inputdata[i][0] < 0) {
             printf("DEALLOCATE MEM: PID %d\n", abs(inputdata[i][0]));
             deallocate_memory(ALLOC_LIST, FREE_LIST, abs(inputdata[i][0]), Memory_Mgt_Policy);
       }
       else {
             printf("COALESE/COMPACT\n");
             FREE_LIST = coalese_memory(FREE_LIST);
       }   
     
       printf("************************\n");
       print_list(FREE_LIST, "Free Memory");
       print_list(ALLOC_LIST,"\nAllocated Memory");
       printf("\n\n");
   }
  
   list_free(FREE_LIST);
   list_free(ALLOC_LIST);
  
   return 0;
}